...
def test_duplicate_fingerprint_reuses_one_job(tmp_path):
    ...
    first = store.enqueue(record(), fingerprint="fp-1")
    second = store.enqueue(CandidateRecord(name="另一条"), fingerprint="fp-1")

    assert first.id == second.id
    assert store.get_by_fingerprint("fp-1").payload["name"] == "张三"


def test_rds_failure_blocks_feishu(tmp_path):
    store = DeliveryStore(tmp_path / "outbox.db")
    store.enqueue(record(), fingerprint="fp-2")
...
def test_feishu_failure_retries_only_feishu(tmp_path):
    store = DeliveryStore(tmp_path / "outbox.db")
    store.enqueue(record(), fingerprint="fp-3")
    claimed = store.claim()[0]
    rds = FakeRds()
...
def test_missing_feishu_record_id_is_not_success(tmp_path):
    store = DeliveryStore(tmp_path / "outbox.db")
    store.enqueue(record(), fingerprint="fp-4")
    job = store.claim()[0]
...
def test_expired_lease_recovers_after_process_crash(tmp_path):
    store = DeliveryStore(tmp_path / "outbox.db")
    store.enqueue(record(), fingerprint="fp-crash")
